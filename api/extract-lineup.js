...
export default async function handler(request,response){
 ...
 const{imageDataUrl,expectedTeam='',expectedTeams=[],sourceType='team-sheet',bothTeams=false}=request.body||{};
 ...
 const schema=bothTeams?{type:'object',additionalProperties:false,properties:{lineups:{type:'array',minItems:2,maxItems:2,items:LINEUP_SCHEMA}},required:['lineups']}:LINEUP_SCHEMA;
 try{
  const expected=bothTeams?(Array.isArray(expectedTeams)?expectedTeams.join(' and '):'both match teams'):(expectedTeam||'unknown');
  const instruction=bothTeams?'Return exactly two separately structured lineups from the image. Do not combine their players.':'Return the team most closely matching the expected team.';
  const apiResponse=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:process.env.OPENAI_LINEUP_MODEL||'gpt-5.6-luna',reasoning:{effort:'low'},input:[{role:'user',content:[{type:'input_text',text:`Extract the football lineup from this ${sourceType}. Expected team or teams: ${expected}. Preserve printed player names and shirt numbers exactly. Identify starters, substitutes, goalkeeper markers and captain markers. Do not guess unreadable text: use an empty number or name fragment and explain it in uncertainties. ${instruction} The scout will review every field before acceptance.`},{type:'input_image',image_url:imageDataUrl,detail:'high'}]}],text:{format:{type:'json_schema',name:bothTeams?'football_lineups':'football_lineup',strict:true,schema}}})});
  ...
  return response.status(200).json({...(bothTeams?{lineups:parsed.lineups}:{lineup:parsed}),model:payload.model||process.env.OPENAI_LINEUP_MODEL||'gpt-5.6-luna'});
