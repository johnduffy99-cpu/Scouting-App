...
try {
 for(const period of ['pre','first','halftime','second','fulltime','terminated']){
  ...
  await page.locator('#new-match').waitFor();assert.match(await page.locator('.release-version').innerText(),/App v1\.8\.1/);
  ...
  await page.locator('#resume').click();assert.equal(await page.locator('#new-match').count(),0);assert.match(await page.locator('.release-version').innerText(),/App v1\.8\.1/);
